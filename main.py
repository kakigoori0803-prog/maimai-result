...
import json, os, hashlib, csv, re, html as htmlmod
...
def _find(pat, src):  # helper
    m = re.search(pat, src, re.I | re.S)
    return (m.group(1).strip() if m else "")

def parse_detail_html(html: str, url: str) -> dict:
    # 平文を作る（タグ除去・実体参照復元）
    plain = htmlmod.unescape(_strip_tags(html))

    # ---- title ----
    title = (
        _find(r'name=["\']music_title["\'][^>]*value=["\']([^"\']+)["\']', html) or
        _find(r'<input[^>]*type=["\']text["\'][^>]*value=["\']([^"\']+)["\']', html) or
        _find(r'<meta[^>]+property=["\']og:title["\'][^>]+content=["\']([^"\']+)["\']', html) or
        _find(r'<title[^>]*>\s*([^<]{2,})\s*</title>', html) or
        _find(r'^\s*([^\n]{2,80})\s*$', plain)  # 最後の保険
    )
    ...
    # 101.0000% / 全角％ どちらも許可
    rate = (
        _find(r'([0-9]{2,3}\.[0-9]{4})\s*[%％]', plain) or
        _find(r'ACHIEVEMENT[^0-9]*([0-9]{2,3}\.[0-9]{4})', plain)
    )

    # ---- playedAt ----
    played = _find(r'(\d{4}/\d{2}/\d{2}\s+\d{2}:\d{2})', plain)

    # ---- difficulty / level ----
    difficulty = _find(r'\b(Re:?MASTER|MASTER|EXPERT|ADVANCED|BASIC)\b', plain)
    level      = _find(r'LEVEL[^0-9]*([0-9]{1,2}\+?)', plain)

    # ---- image (相対パス→絶対URL) ----
    img = (
        _find(r'<img[^>]+src=["\']([^"\']+?(?:png|jpg|jpeg|webp))["\']', html) or
        _find(r'background-image:\s*url\(["\']?([^"\')]+)["\']?\)', html)
    )
    imageUrl = urljoin(url or "", img) if img else ""
    ...
    if isinstance(items, list) and items:
        ...
        for item in items:
            ...
            if not any(r.get("uniq")==uniq for r in db):
                item["uniq"]=uniq; item["sourceUrl"]=item.get("sourceUrl") or src
        ...
        item = parse_detail_html(body.get("html") or "", url)
        if item.get("rate") or item.get("playedAt") or item.get("title"):
            key  = f"{item.get('title','')}|{item.get('rate','')}|{item.get('playedAt','')}"
            uniq = hashlib.sha1(key.encode()).hexdigest()
            if not any(r.get("uniq")==uniq for r in db):
                item["uniq"]=uniq; item["ingestedAt"]=now
                db.append(item); inserted += 1
    else:
        raise HTTPException(status_code=400,
            detail="Payload must be {items:[...]} or {html:..., url:...}")
    save_db(db); return inserted
...
async def ingest_form(request: Request, token: str = Query("")):
    if token != API_TOKEN:
        raise HTTPException(status_code=401, detail="Unauthorized (token)")
    raw = await request.body()
    if not raw: raise HTTPException(status_code=400, detail="Empty body")
    ctype = request.headers.get("Content-Type","")
    if "application/x-www-form-urlencoded" in ctype:
        d = parse_qs(raw.decode("utf-8"))
        payload = (d.get("payload") or [""])[0]
        if not payload: raise HTTPException(status_code=400, detail="Missing 'payload'")
        try: body = json.loads(payload)
        except Exception as e: raise HTTPException(status_code=400, detail=f"Invalid JSON in 'payload': {e}")
    else:
        try: body = json.loads(raw.decode("utf-8"))
        except Exception as e: raise HTTPException(status_code=400, detail=f"Invalid JSON: {e}")
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    inserted = ingest_from_body(body, now)
    return JSONResponse({"status":"ok","inserted":inserted,"total":len(load_db())})
...
def data_csv():
    buf = StringIO()
    fields = ["playedAt","title","difficulty","level","rate","imageUrl","ingestedAt","sourceUrl"]
    w = csv.DictWriter(buf, fieldnames=fields); w.writeheader()
    for r in load_db(): w.writerow({k:r.get(k,"") for k in fields})
...
def diff_badge(d, level=None):
    d_raw = (d or "").strip(); d_l = d_raw.lower()
    ...
    elif "re:master" in d_l or "remaster" in d_l or d_l=="re":
...
def view():
    data = load_db(); data.sort(key=lambda r:r.get("ingestedAt",""), reverse=True)
    groups = {}
    for r in data:
    ...
    for d,rows in sorted(groups.items(), key=lambda x:x[0], reverse=True):
        ...
        for r in rows:
            ...
            level=r.get("level"); rate=r.get("rate","")
            rate_txt=human_rate(rate); rate_cls=rank_class(rate)
...
<section class='card'>
  <h2>{esc(d) or '未日付'}</h2>
  <ul class='list'>{''.join(rows_html)}</ul>
</section>""")
    html=f"""<!doctype html>
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>maimai result</title>
<style>
  :root {{ --bg:#0f172a; --card:#111827; --text:#e5e7eb; --muted:#94a3b8; --accent:linear-gradient(135deg,#06b6d4,#8b5cf6); }}
  *{{box-sizing:border-box}} body{{margin:0;font-family:-apple-system,system-ui,Segoe UI,Roboto,'Hiragino Kaku Gothic ProN','Noto Sans JP',sans-serif;background:var(--bg);color:var(--text);
  background-image:radial-gradient(ellipse at top,rgba(99,102,241,.15),transparent 40%),radial-gradient(ellipse at bottom,rgba(20,184,166,.15),transparent 40%);}}
  header{{display:flex;align-items:center;justify-content:space-between;padding:18px 14px;position:sticky;top:0;z-index:10;backdrop-filter:blur(10px);
  background:linear-gradient(180deg,rgba(15,23,42,.85),rgba(15,23,42,.55));border-bottom:1px solid rgba(255,255,255,.06)}}
  .logo{{height:28px}} .logo-text{{font-weight:800;font-size:20px;background:var(--accent);-webkit-background-clip:text;background-clip:text;color:transparent}}
  .toolbar a{{color:var(--muted);text-decoration:none;margin-left:14px;font-size:13px}} main{{padding:14px}}
  .card{{background:var(--card);border:1px solid rgba(255,255,255,.06);border-radius:14px;padding:8px 10px;margin:10px 0 14px;box-shadow:0 6px 20px rgba(0,0,0,.25)}}
  h2{{margin:8px 6px 4px;font-size:14px;color:var(--muted);font-weight:600}} .list{{list-style:none;padding:0;margin:0}}
  .row{{display:flex;align-items:center;justify-content:space-between;padding:10px 8px;border-top:1px solid rgba(255,255,255,.06);gap:10px}}
  .row:first-child{{border-top:none}} .left{{display:flex;gap:10px;align-items:center;min-width:0}} .txt{{min-width:0}}
  .left .title{{font-size:15px;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:62vw}}
  .left .meta{{font-size:12px;color:var(--muted);margin-top:2px}} .right{{font-variant-numeric:tabular-nums;font-size:16px;font-weight:700;text-align:right;min-width:88px}}
  .jacket{{width:44px;height:44px;border-radius:8px;flex:0 0 auto;object-fit:cover;border:1px solid rgba(255,255,255,.08);background:#0b1220}}
  ...
  .rk-sss{{color:#f97316}} .rk-ssplus{{color:#eab308}} .rk-ss{{color:#facc15}} .rk-splus{{color:#06b6d4}} .rk-s{{color:#3b82f6}} .rk-none{{color:#e5e7eb}}
</style>
<header>{logo_html}<nav class="toolbar"><a href="/data/pretty">JSON</a><a href="/data.csv">CSV</a><a href="/health">health</a></nav></header>
<main>{(''.join(cards) if cards else '<div style="text-align:center;color:#94a3b8;padding:40px 8px;">データがありません。ショートカットから同期してね。</div>')}</main>"""
